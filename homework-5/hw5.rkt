  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([c (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? c)
               (let ([fname (fun-nameopt (closure-fun c))]
                     [fenv (cons (cons (fun-formal (closure-fun c)) arg) (closure-env c))])
                 (eval-under-env (fun-body (closure-fun c)) (if fname
                                                                (cons (cons fname c) fenv)
                                                                fenv)))
...
(define (compute-free-vars e)
  (define (get-vars e)
    (cond [(var? e) (set (var-string e))]
          [(add? e) (set-union (get-vars (add-e1 e)) (get-vars (add-e2 e)))]
          [(int? e) (set)]
          [(aunit? e) (set)]
          [(isaunit? e) (get-vars (isaunit-e e))]
          [(apair? e) (set-union (get-vars (apair-e1 e)) (get-vars (apair-e2 e)))]
          ...
          [(mlet? e) (set-union (get-vars (mlet-e e)) (set-remove (get-vars (mlet-body e)) (mlet-var e)))]
          [(closure? e) (get-vars (closure-fun e))]
          [(fun? e)
           (let ([vs (set-remove (get-vars (fun-body e)) (fun-formal e))])
             (if (fun-nameopt e)
                 (set-remove vs (fun-nameopt e))
                 vs))]
          [(call? e) (set-union (get-vars (call-funexp e)) (get-vars (call-actual e)))]
          [#t (error (format "bad MUPL expression: ~v" e))]))
  (cond [(var? e) e]
        ...
        [(fun? e) (fun-challenge (fun-nameopt e) (fun-formal e) (compute-free-vars (fun-body e)) (get-vars e))]
...
(define (eval-under-env-c e env)
  (define (reduce-env env freevars)
    (if (null? env)
        null
        (if (set-member? freevars (caar env))
            (cons (car env) (reduce-env (cdr env) freevars))
            (reduce-env (cdr env) freevars))))
  (cond [(var? e) 
         (envlookup env (var-string e))]
        [(add? e) 
        ...
        [(closure? e) e]
        [(fun-challenge? e)
         (closure (reduce-env env (fun-challenge-freevars e)) e)]
        [(call? e)
         (let ([c (eval-under-env-c (call-funexp e) env)]
               [arg (eval-under-env-c (call-actual e) env)])
           (if (closure? c)
               (let ([fname (fun-challenge-nameopt (closure-fun c))]
                     [fenv (cons (cons (fun-challenge-formal (closure-fun c)) arg) (closure-env c))])
                 (eval-under-env-c (fun-challenge-body (closure-fun c)) (if fname
                                                                            (cons (cons fname c) fenv)
                                                                            fenv)))
               (error "MUPL call applied to non-function")))]
        [#t (error (format "bad MUPL expression: ~v" e))]))
